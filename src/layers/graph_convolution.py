...
class GraphConvolutionLayer(tf.keras.layers.Layer):
    ...
    def call(self, adjacency_matrix, node_features):
        degree_matrix_inv_sqrt = tf.linalg.diag(1.0 / tf.sqrt(tf.reduce_sum(adjacency_matrix, axis=-1)))
        normalized_adj = tf.matmul(tf.matmul(degree_matrix_inv_sqrt, adjacency_matrix), degree_matrix_inv_sqrt)
